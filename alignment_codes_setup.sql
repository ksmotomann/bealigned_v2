-- Alignment Codes System Setup
-- Run this script directly in Supabase SQL Editor

-- 1. Create alignment codes table
...
-- 3. Create user-alignment code association table for individual tracking
...
-- 5. RLS Policies for alignment_codes
...
-- 6. RLS Policies for user_alignment_codes
...
-- 7. Function to validate and use an alignment code
...
-- 8. Helper functions for trial management
...
-- 9. Create updated_at trigger for alignment_codes
...
-- 10. Success message
SELECT 'Enhanced alignment codes system with user associations and expiration management setup complete!' as status;

-- Example queries to test the system:
-- 1. Check if a user can use a new code:
-- SELECT * FROM public.can_user_use_new_code('USER_UUID_HERE');
--
-- 2. Get user trial status:
-- SELECT * FROM public.get_user_trial_status('USER_UUID_HERE');
--
-- 3. Expire old codes (run periodically):
-- SELECT public.expire_alignment_codes();
--
-- 4. View all user alignment code associations:
-- SELECT * FROM public.user_alignment_codes ORDER BY used_at DESC;