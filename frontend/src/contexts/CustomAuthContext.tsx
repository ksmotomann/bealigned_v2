...
import { UserRole, FEATURE_FLAGS, hasPermission, hasFeatureAccess } from '../types/roles';
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const signOut = async () => {
    ...
    // Redirect to sign-out page after successful sign out
  ...
  const updateProfile = async (firstName: string, lastName: string) => {
    const { error } = await authService.updateProfile(firstName, lastName);
    if (error) throw new Error(error);
  };
  ...
  // Backward compatibility - calculate inside render for reactivity
  ...
  useEffect(() => {
    if (user) {
      console.log('CustomAuthContext Debug:', {
